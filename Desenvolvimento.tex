...
Analisando a Figura \ref{img_pArq_muxesUla}, percebe-se que o primeiro multiplexador, o qual recebe o nome de \textit{muxOp1}. Ele recebe conexões dos registradores \textit{regJump} (que será abordado no Capítulo \ref{modificacoes}) e \textit{regOp1}. Ele é um multiplexador com 4 entradas pois a ideia inicial era utilizá-lo para receber valores, quais aparecem na figura. Contudo, durante a fase de modificações, optou-se por aumentar o número de \bis{} no seletor da ULA e reservar códigos para que fossem feitos os incrementos necessários. Sendo assim, as duas entradas remanescentes desse MUX estão vazias e poderão ser utilizadas para o acoplamento de novas funcionalidades.
...
Caso fosse priorizada a reutilização de estados, alguns dos passos que estão sendo feitos em apenas dois estados, poderiam ser feitos em três ou mais. Isso porque cada um dos estados teriam que ser menos complexos, visando que seja possível reutilizá-los em mais de uma instrução. Porém, como eles são complexos e cheios de sinais de saída específicos para cada instrução, torna-se extremamente difícil reutilizá-los em outra instrução sem perder desempenho.

Existe ainda uma funcionalidade que é básica e feita no primeiro estado de cada instrução. Tal funcionalidade é a que permite a leitura, vinda da memória de instruções, dos 2 \bys{} de uma instrução: \textit{OpArg} e \textit{OpCode}. 
...
Em todas as máquinas de estados, foi inserido um estado auxiliar que permite ao controle ler o valor do \textit{regInstr} e tomar a decisão sobre qual é o próximo estado. A função deste estado é basicamente esperar e, para isso, todos os sinais de controle ficam desativados. Em nenhuma das máquinas de estados criadas foi mostrado esse estado, mas na implementação é possível percebê-lo.
