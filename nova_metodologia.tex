...
A fase de desenvolvimento fala apenas sobre a criação do \hw{}, visto que o \textit{pyConv} foi uma implementado para ser uma ferramenta extra, utilizado, principalmente, para gerar arquivos de testestestes. A criação do conversor, tanto quanto seus métodos para evitar erros, são explicados no capítulo \ref{conversor}.
...
Para o levantamento dos resultados, foram analisados, principalmente, dados gerados pelas ferramentas utilizadas. Tanto o \textit{Quartus} quanto o \textit{Modelsim} gerando dados que permitem que uma análise sólida do funcionamento da arquitetura seja feita.

O \textit{Quartus} gera dados relacionados à arquitetura em si, permitindo que seja possível verificar dados como área ocupada em um \textit{chip} (quantidade de componentes utilizados) e características básicas, como frequência de operação e energia consumida. Ainda com essa ferramenta é possível realizar simulações, contudo essa não é uma boa funcionalidade para ser utilizada tendo em vista a falta de dados apresentados nas simulações.

As simulações do \textit{Quartus} apenas apresentam dados dos pinos de entrada e saída, não sendo possível analisar dados presentes internamente na arquitetura. Sendo assim, para este quesito, a ferramenta \textit{Modelsim} entra em ação.

Tal ferramenta permite que sejam verificados dados internos aos componentes, facilitando, principalmente, o encontro das causas de problemas na execução de instruções. Um grande exemplo é o fato de algum algoritmo executado não realizar um salvamento na memória, algo que era esperado. Com a ferramenta, é possível identificar o exato componente problemático através de análises de ondas geradas pela execução, indicando os valores presentes em cada um dos sinais internos.

Assim, dividiu-se a utilização do \textit{Quartus} apenas para verificar características da arquitetura e inicializar a mesma em um FPGA, também utilizado para testes. Já com o \textit{Modelsim} ficou toda a parte de simulação e correção de \textit{bugs} encontrados. Somente quando tem-se simulações perfeitas no \textit{Modelsim} que passa-se ao \textit{Quartus} para realizar testes com um FPGA.